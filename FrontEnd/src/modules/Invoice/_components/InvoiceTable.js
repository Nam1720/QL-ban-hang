...
import { setListInvoice, setModalInfo, setModalRemove } from '../_store/invoiceSlice';
import { openNotificationWithIcon, formatNumber } from '../../../helpers/funcs';
...
const InvoiceTable = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: text => <span style={{ color: 'red' }}>{text ? formatNumber(text) : text}</span>
    },
    {
      title: 'Hoạt động',
      key: 'action',
      render: (text, record) => (
        <Space size="middle">
          <Button onClick={() => handleClickInfo(record)} type="primary" shape="round" size='small'>Thông tin</Button>
          <Button onClick={() => handleClickRemove(record.codeInvoice)} type="primary" shape="round" size='small' danger>Xóa</Button>
        </Space>
      ),
    },
  ];

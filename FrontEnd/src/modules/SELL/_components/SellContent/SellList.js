    ...
    productsBuying.map((o) => {
      if (o.codeProduct === value.codeProduct) {
        if (o.inventory >= e) {
          ...
          openNotificationWithIcon(
            'error',
            `Sản phảm ${o.productName} hết hàng!`
          );
          newArray.push(o);
          dispatch(setProductsBuying([...newArray]));
        }
          ...
          <div className="item d-flex-center">
            ...
            <div className="item__price d-flex align-items-center">
              <InputNumber
                ...
                value={value.amout}
                min={1}
                onChange={(e) => handleChangeAmout(e, value)}
              />
              <InputNumber
                style={{ width: '100px', border: 'none' }}
                value={value.priceSell}
                onChange={(e) => handleChangePriceSell(e, value)}
                formatter={(value) =>
              ...
              <button onClick={() => handleClickRemove(value.codeProduct)}>
                <DeleteOutlined
